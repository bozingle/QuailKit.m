...
switch task
    case 'reset'
        if initialize
            ...
            handles=Set123(handles,true);
            handles=Set45(handles,true);
            handles.Graphics.Credits=Credits(handles.Graphics.Watermark,handles.Graphics.Color,true);
            handles.Graphics.Watermark=HT_Transition(handles.Graphics.Watermark,...
                {'YLim'},[1-handles.Graphics.Watermark.Position(4),0],frames);
    ...
    case {'default', 'GroupPlay'}
        ...
        handles=SetToolbar(handles);
        handles=SetPlay(handles);
        handles=Set123(handles,false);
        handles=Set45(handles,false);
        handles=Wait(handles,'off',frames);
    case 'init'
        handles=SetAxis(handles,initialize);
        handles=SetGraphics(handles);
    ...
    case 'Toolbar'
        handles=SetToolbar(handles);
    case 'OneTwo'
        handles=Set123(handles,initialize);
    case 'ThreeFour'
        handles=Set45(handles,initialize);
end
if nargout==0
...
h=repmat(handles.Graphics.Watermark,1,3);
switch state
    case 'on'
        handles=Lock(handles,'on');
        handles.Graphics.Credits.Visible='on';
        h=HT_Transition(h,{'XLim','YLim','Alphamap'},...
            [round([1,h(1).Position(3),1,h(1).Position(4)]),0.9,0.98],frames);
        handles.Graphics.Watermark=h(1);
    case 'off'
        h=HT_Transition(h,{'XLim','YLim','Alphamap'},...
            round([h(1).Position(3)/2+[-20,-18],h(1).Position(4)/2+[18,20],0,0.1]),frames);
        handles.Graphics.Watermark=h(1);
...
switch state
    case 'on'
        h=findobj(handles.Fig,{'Enable','on','-and','-not','Parent',handles.Toolbar});
        set(h,'Enable','inactive');
        h=findobj(handles.Fig,{'Enable','on','-and','Parent',handles.Toolbar});
        set(h,'Enable','off');
    case 'off'
        h=findobj(handles.Fig,{'Enable','inactive'});
        set(h,'Enable','on');
        h=findobj(handles.Fig,{'Visible','on','-and','Parent',handles.Toolbar});
        set(h,'Enable','on');
end
end
...
function handles=Set123(handles,initialize)
if initialize
...
switch handles.Two_Pop.Value
    ...
    case {2,3}
        set(handles.Two.Children,'Enable','on');
        handles.Two_List.String=cell(size(handles.Two_Pop.UserData{handles.Two_Pop.Value,2},1),1);
        for i=1:size(handles.Two_Pop.UserData{handles.Two_Pop.Value,2},1)
            switch class(handles.Two_Pop.UserData{handles.Two_Pop.Value,2}{i,2})
                case {'char','string'}
                    format='%s';
                case 'logical'
                    format='%d';
            ...
            handles.Two_List.String{i}=...
                sprintf(['%s: ',format],handles.Two_Pop.UserData{handles.Two_Pop.Value,2}{i,1},...
                handles.Two_Pop.UserData{handles.Two_Pop.Value,2}{i,2});
        end
end
if handles.Two_Pop.Value>1
    switch class(handles.Two_Pop.UserData{handles.Two_Pop.Value,2}{handles.Two_List.Value,2})
        case 'double'
...
function handles=Set45(handles,initialize)
temp=SH_FindCalls();
...
flag2=0;
if handles.Data.TS.Time(handles.Data.Edges(handles.Data.j+1))>handles.Graphics.Patch(1,1,1).XData(1)
    flag2=1;
end
...
switch handles.UserData.ZoomMode
    case 0
        ...
        Trans(7)=0.1;
    case 1
        Trans(1:2)=[handles.Data.TS.Time(handles.Data.Edges(handles.Data.j))-295,...
            handles.Data.TS.Time(handles.Data.Edges(handles.Data.j+1))+295];
        Trans(3:4)=max(abs(handles.Graphics.Plot(1,1,1).YData(round(...
            max(2,0.001*(handles.Data.Edges(handles.Data.j)-295*handles.Data.fs)):...
            min(length(handles.Graphics.Plot(1,1,1).XData),...
            0.001*(handles.Data.Edges(handles.Data.j+1)+295*handles.Data.fs))))))*[-1,1];%workaround
        Trans(7)=0.1;
    case 2
...
if ismember(handles.UserData.ZoomMode,[2,3])
    if ~flag2
        set(handles.Graphics.Patch(1,active,1),...
            ...
            [handles.Data.Edges(handles.Data.j-flag2),handles.Data.Edges(handles.Data.j+2-flag2),...
            handles.Data.Edges(handles.Data.j+2-flag2),handles.Data.Edges(handles.Data.j-flag2)]));
        for k=active
            set(handles.Graphics.Plot(1,k,2),...
                'XData',handles.Data.TS.Time(...
                handles.Data.Edges(handles.Data.j-flag2):handles.Data.Edges(handles.Data.j+2-flag2)),...
                'YData',handles.Data.TS.Data(...
                handles.Data.Edges(handles.Data.j-flag2):handles.Data.Edges(handles.Data.j+2-flag2),k));
        end
    end
else
...
for k=active
    if ~flag && ~ismember(handles.UserData.ZoomMode,[2,3])
        set(handles.Graphics.Patch(1,k,1),...
            'XData',handles.Data.TS.Time(...
            [handles.Data.Edges(handles.Data.j),handles.Data.Edges(handles.Data.j+1),...
            handles.Data.Edges(handles.Data.j+1),handles.Data.Edges(handles.Data.j)]));
        set(handles.Graphics.Line(1,k,1),...
            'XData',handles.Data.TS.Time(handles.Data.Edges(handles.Data.j))*[1,1],...
    ...
    if ismember(handles.UserData.ZoomMode,[2,3])
        set(handles.Graphics.Plot(1,k,2),...
            ...
            handles.Data.Edges(handles.Data.j):handles.Data.Edges(handles.Data.j+1)),...
            'YData',handles.Data.TS.Data(...
            handles.Data.Edges(handles.Data.j):handles.Data.Edges(handles.Data.j+1),k),...
    ...
    if handles.Four_Pop.UserData
        set(handles.Graphics.Axis(2,k),...
            'XLim',handles.Data.TS.Time([handles.Data.Edges(handles.Data.j),...
            handles.Data.Edges(handles.Data.j+1)]),...
...
if initialize
    ...
    set(handles.Graphics.Surf(:),'XData',NaN(1,2),'YData',NaN(1,2),'ZData',NaN(2,2));
    set(handles.Graphics.Patch(:),'XData',NaN,'YData',NaN);
    set(handles.Graphics.Line(:),'XData',NaN,'YData',NaN,'ZData',NaN);
end
end

function handles=SetLayout(handles,frames)
activeMics=find([handles.Two_Pop.UserData{2,2}{:,2}]);
activeAxes=find([handles.Four_List.UserData{handles.Four_Pop.Value}{2,:}]==true);
inactiveMics=find(~[handles.Two_Pop.UserData{2,2}{:,2}]);
inactiveAxes=find([handles.Four_List.UserData{handles.Four_Pop.Value}{2,:}]==false);
...
handles.Two.Position=[sum(handles.One.Position([1,3]))+s,s,...
    (handles.Group123.Position(3)-(nd+1)*s)/nd,handles.Group123.Position(4)-2*s];
...
handles.Five.Position=[sum(handles.Four.Position([1,3]))+s,s,...
    (handles.Group45.Position(3)-(na+1)*s)/na,handles.Group45.Position(4)-2*s];
...
for i=inactiveAxes
    set([handles.Graphics.Axis(i,:);handles.Graphics.Axis(i,:).Children],'Visible','off');
end
set([reshape(handles.Graphics.Axis(:,inactiveMics),[],1);cat(1,handles.Graphics.Axis(:,inactiveMics).Children)],'Visible','off');
set(handles.Graphics.Watermark,'Position',handles.Panel.Position);
handles.Graphics.Credits=Credits(handles.Graphics.Watermark,handles.Graphics.Color,false);
for l=1:size(Path,1)
    j=1;
    for i=activeAxes
        for k=activeMics
...
for i=activeAxes
    set([handles.Graphics.Axis(i,activeMics);handles.Graphics.Axis(i,activeMics).Children],'Visible','on');
end
end
...
handles.Graphics.Color=handles.Graphics.Colormap(32,:)/(3*max(handles.Graphics.Colormap(32,:)));
handles.icons=handles.Fig.UserData;
temp=fieldnames(handles.icons);
for i=1:length(temp)
    handles.icons.(char(temp(i)))(:,:,1:3:end)=...
        handles.icons.(char(temp(i)))(:,:,1:3:end)*handles.Graphics.Color(1);
    handles.icons.(char(temp(i)))(:,:,2:3:end)=...
        handles.icons.(char(temp(i)))(:,:,2:3:end)*handles.Graphics.Color(2);
    handles.icons.(char(temp(i)))(:,:,3:3:end)=...
        handles.icons.(char(temp(i)))(:,:,3:3:end)*handles.Graphics.Color(3);
end
if nargin>1
    switch varargin{1}
        case 'Toolbar'
    ...
    for i=1:length(handles.Group45.Children)
        set(handles.Group45.Children(i).Children,'ForegroundColor',handles.Graphics.Color);
    end
    for i=1:length(handles.Group123.Children)
        set(handles.Group123.Children(i).Children,'ForegroundColor',handles.Graphics.Color);
    end
    set(handles.Graphics.Plot(:,:,1),'Color',handles.Graphics.Color);
    set(handles.Graphics.Plot(1,:,2),'Color',handles.Graphics.Colormap(32,:));
    set(handles.Graphics.Scatter(3,:,1),'MarkerEdgeColor',handles.Graphics.Color);
    set(handles.Graphics.Line(:),'Color',handles.Graphics.Color);
    set(handles.Graphics.Line(3,:,:),'Color','r');
    set(handles.Graphics.Patch(:),'FaceColor',handles.Graphics.Color,'EdgeColor',handles.Graphics.Color);
    ...
    function handles=SetGraphics_Toolbar(handles)
        ...
        for j=1:length(Buttons)
            if strcmp(handles.(Buttons{j}).State,'off')
                ...
                handles.(Buttons{j}).CData=HT_Feedback(handles.icons.(Buttons{j}));
            end
        end
    ...
    function handles=SetGraphics_GroupPlay(handles)
        ...
        handles.PlayMode.CData=handles.icons.PlayMode(:,:,3*handles.PlayMode.UserData+(1:3));
        handles.Mode.CData=handles.icons.Mode(:,:,3*handles.Mode.UserData+(1:3));
        handles.Debug.CData=handles.icons.Debug(:,:,3*handles.Debug.UserData+(1:3));
    end
end
...
cdata(1+corner(1):m(1)+corner(1),1+corner(2):m(2)+corner(2),:)=logo(:,:,1:3);
for i=1:3
    temp=cdata(:,:,i);
...
alpha(1+corner(1):m(1)+corner(1),1+corner(2):m(2)+corner(2))=b*double(logo(:,:,4));
alpha(alpha==0)=1;
cdata(1+corner(1):m(1)+corner(1),1+corner(2):m(2)+corner(2),:)=logo(:,:,1:3);
for i=1:3
    temp=cdata(:,:,i);
...
if initialize
    ...
    text(h,1,1,sprintf('%s\n',...
        ...
        'Supervisor'),'FontUnits','normalized','FontWeight','normal','Color',[1,1,1],...
        'HorizontalAlignment','center');
    text(h,1,1,sprintf('%s\n',...
        ' ',' ','Hanif Tiznobake',' ',' ',...
        'Stephen Huang','Hanif Tiznobake',' ',' ',...
        'Hamed Sari-Sarraf'),'FontUnits','normalized','FontWeight','bold','Color',[1,1,1],...
        'HorizontalAlignment','center');
else
    p=h.Children(end);
    set(p,...
