SQL=[
    'SELECT [name], DATEDIFF(millisecond,an.[start], detection.[start])/1000.0 as [start], DATEDIFF(millisecond,an.[start], detection.[end])/1000.0 as [end] ',...
    'FROM detection inner join data on detection.data_id=data.stream_id inner join audio_data ad on detection.data_id=ad.data_id inner join audio_node an on ad.audio_id=an.audio_id ',...
    'WHERE DATEDIFF(millisecond,an.[start], detection.[start])/1000.0 >240'];
d=HT_DataAccess([],'query',SQL,'cellarray');

doa=[];
for i =1:size(d,1)
    obj = JR_Data('',fullfile('Z:','QuailKit','data',d{i,1}),40,0:10:10000,0.8);    
